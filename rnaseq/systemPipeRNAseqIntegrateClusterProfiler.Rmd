...
author: "Gordon Mosher (gmosh001@ucr.edu)"
...
doallchunks = TRUE # 2017-0731 now 24m (17 real) on intel partition with ALL chunks on, 245 pages
#doallchunks = FALSE
FALSEdoallchunks = FALSE # place holder for doallchunks that is always FALSE

doallClusterRunchunks = TRUE
#doallClusterRunchunks = FALSE
FALSEdoallClusterRunchunks = FALSE # place holder for ... that is always FALSE

doallCPchunks = TRUE # if only doall, takes 11m, 126 pages
#doallCPchunks = FALSE
FALSEdoallCPchunks = FALSE # place holder for ... that is always FALSE

# cat <- function(...) {} # use this to disable all cat() calls below (for windows)
displayKnitStatus <- function() {
	system("echo HOSTNAME HOSTNAME HOSTNAME HOSTNAME HOSTNAME HOSTNAME HOSTNAME")
	system("hostname") # should return name of a compute node starting with i or c 
	system("date")
	system("echo ==============================================================")
...
rm(BatchResult); rm(BatchResultslim); rm(catdb)
save.image()
```
...
```{r fastq_filter, eval=FALSEdoallchunks}
displayKnitStatus()
...
showClass("SYSargs")
# next line takes approx 6 hours on alldata
...
```{r fastq_report, eval=FALSEdoallchunks}
displayKnitStatus()
#knitr::knit_exit()
...
cat('Next line takes time 6 minutes.\n') # takes 6 minutes, takes 25 min for alldata
displayKnitStatus()
fqlist <- systemPipeR::seeFastq(fastq=infile1(args), batchsize=100000, klength=8)
displayKnitStatus()
save(fqlist, file="data/GO/fqlist.RData")  # GO folder must exist or this fails
...
```{r tophat_alignment2, eval=FALSEdoallClusterRunchunks}
displayKnitStatus()
# ?moduleload
# q
args
...
reg <- systemPipeR::clusterRun(args, conffile=".BatchJobs.R", template="slurm.tmpl", Njobs=18, runid="01",
                  resourceList=resources)  # takes about 63 min for alldata
...
reg <- clusterRun(args, conffile=".BatchJobs.R", template="slurm.tmpl", Njobs=18, runid="01",
                  resourceList=resources)  # takes about 30 min for alldata
# don't submit for 20 hours on short-list!
# 1 minute
reg
waitForJobs(reg)
...
#knitr::knit_exit()
```
```{r read_counting2, eval=FALSEdoallchunks} # was TRUE until 2019-0304
displayKnitStatus()
# takes about 30 min for alldata
...
# desc <- biomaRt::getBM(attributes=c("band", "description"), mart=m)
...
```{r include_systemPipeR, eval=TRUE}
library(GO.db)
library(AnnotationDbi)
source("../../systemPipeR/R/AllClasses.R")
source("../../systemPipeR/R/fastqQuality.R")
source("../../systemPipeR/R/overLapper.R")
source("../../systemPipeR/R/utilities.R")
source("../../systemPipeR/R/geneSetAnalysis.R")
...
listDatasets(m)
listDatasets(m)[45,] # athaliana_eg_gene
m <- useMart("plants_mart", dataset="athaliana_eg_gene", host="plants.ensembl.org")
listAttributes(m) # Choose data types you want to download
go <- biomaRt::getBM(attributes=c("go_id", "tair_locus", "name_1006"), mart=m)
str(go)
...
m <- useMart("ensembl", dataset="xmaculatus_gene_ensembl", host="ensembl.org")
listAttributes(m)
go <- biomaRt::getBM(attributes=c("go_id", "band", "name_1006"), mart=m)
go <- go[go[,3]!="",]; go[,3] <- as.character(go[,3])
go[go[,3]=="molecular_function", 3] <- "F"; go[go[,3]=="biological_process", 3] <- "P"; go[go[,3]=="cellular_component", 3] <- "C"
go[1:4,]
...
DEGlist
save(DEGlist, file="DEGlist.RData") # save for clusterProfiler::compareClusters work
debug(GOCluster_Report)
...
m <- useMart("plants_mart", dataset="athaliana_eg_gene", host="plants.ensembl.org")
# m <- useMart("ensembl", dataset="xmaculatus_gene_ensembl", host="ensembl.org")
goslimvec <- as.character(biomaRt::getBM(attributes=c("goslim_goa_accession"), mart=m)[,1])
goslimvec
...
```{r try.clusterProfiler.pdf.pg5.compareCluster, eval=FALSEdoallCPchunks}
# takes time 5:00
...
fun='groupGO', OrgDb='org.Hs.eg.db') # takes a minute or so
str(xx.formula) # 44 obs. of 7 variables
...
str(xx.formula.twogroups) # 88 obs. of 8 variables
...
str(xxx) # 14 obs. of 10 variables
as.data.frame(xxx)


```
```{r try.clusterProfiler.pdf.pg11.enrichGO, eval=FALSEdoallCPchunks} # FALSE as of 2019-0304, I believe Yu changed his enrichGO(), error = unused arg (keytype="TAIR")
# takes time 1:15
displayKnitStatus()
...
ego = yyy # ego is for visualization
str(yyy)
# yyy@result
str(yyy@result)  # commented 2018-0620 no more slots on this object
yyy@result[,c(1:7,9)]  # commented 2018-0620 no more slots on this object
# head(summary(yyy))
...
```
```{r clusterProfiler.ego.Visualization, eval=T}
load("ego.RData")
data(geneList)
# clusterProfiler Visualization
barplot(ego, showCategory=8)
dotplot(ego)
DOSE::enrichMap(ego) # will be removed in next version, use emapplot
emapplot(ego)
cnetplot(ego, categorySize="pvalue", foldChange=geneList)
plotGOgraph(ego)


# and run all of the clusterProfiler Visualizations again to save to file
# if this takes too much time, use the html code to display the files
...
```
```{r try.clusterProfiler.pdf.pg16.groupGO, eval=FALSEdoallCPchunks} # FALSE as of 2019-0304, I believe Yu changed his groupGO(), error = unused arg (keytype="TAIR") 
# takes time 2:00 
displayKnitStatus()
...
# my code
rebuild_DEGlist_deprecated <- function() {
...
ggo = yyyyBP # ggo is for the visualization
head(summary(yyyyBP))
...
head(summary(yyyyMF))
save(ggo, file="ggo.RData") # save for clusterProfiler visualization
```
