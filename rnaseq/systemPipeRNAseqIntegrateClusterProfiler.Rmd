...
srun --x11 --partition=intel --mem=2gb --cpus-per-task 1 --ntasks 1 --time 30-0:00:00 --pty bash -l
...
cat('fqlist is a Large list (18 elements, 1.3 Mb)\n')
fqlist[[1]][3] # lots of data here, best to visualize with seeFastqPlot?
fqlist[['M1A']][1]
fqlist[['M1A']][2]
fqlist[['M1A']][3]
fqlist[['M1A']][4]
...
```{r tophat_alignment2, eval=doallchunks}
# ?moduleload
# q
args
modules(args)
...
```{r hisat_alignment2, eval=doallchunks}
...
cat(system(paste0('cat data/tair10.gff'), intern = T), sep = '\n')
...
```{r try.clusterProfiler.pdf.pg5.compareCluster, eval=T}
# takes time 2:25
...
```
```{r try.clusterProfiler.pdf.pg11.enrichGO, eval=T}
# takes time :40
...
head(yy)
plot(yy) # where does this output to?
## End(Not run)


```
```{r try.clusterProfiler.pdf.pg16.groupGO, eval=T}
# takes time :30
